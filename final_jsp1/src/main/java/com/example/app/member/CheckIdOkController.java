...
public class CheckIdOkController implements Execute {
	...
	public Result execute(HttpServletRequest request, HttpServletResponse response)
        ...
        // JSON 형식 응답 설정
        response.setContentType("application/json"); // json 응답으로 타입변환
        response.setCharacterEncoding("UTF-8"); // UTF-8 = 한글

        // JSON 응답 작성
        try (PrintWriter out = response.getWriter()) { // PrintWriter = 파일입출력
