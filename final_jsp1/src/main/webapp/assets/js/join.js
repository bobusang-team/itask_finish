document.addEventListener("DOMContentLoaded", function () {
    ...
    sendSMSBtn.addEventListener("click", function () {
        ...
        fetch("/member/sendSMS.me", {
			...
			body: JSON.stringify({ phoneNumber: phoneNumber }) // js객체의 phoneNumber를 JSON으로 보낸다
			// js객체를 json문자열로 변환하여 서버에 전송하기 위함(서버에서 json 데이터를 받도록 설계된 경우 필요)
			// js : 스크립트 언어, 프로그래밍 로직, 객체(Object), 코드 실행 가능, 함수 포함 가능
			// json : 텍스트(String), 데이터 저장, APi 통신(서버와 통신)을 위해서 사용
			// json을 JavaScript에서 사용하려면 JSON.parse()로 변환
			// Javascript 객체를 JSON으로 내보내려면 JSON.stringify() 사용
