...
public class MypageDAO {
	...
	// 해당 유저가 작성한 게시글 총 수
	public int getTotalArticleNum(int userNum) {
		return sqlSession.selectOne("mypage.getTotalArticleNum", userNum);
	}

	// 해당 유저가 작성한 게시글 모두 가져오기
	public List<ArticleListDTO> selectAllMyArticle(Map<String, Integer> pageMap) {
		System.out.println("========selectAll : 유저가 작성한 게시글 가져오기");
		List<ArticleListDTO> myList = sqlSession.selectList("mypage.selectAllMyArticle", pageMap);
		System.out.println("가져온 목록 : " + myList);
		return myList;
	}

	// 해당 유저가 작성한 댓글의 총 수
	public int getTotalCommentNum(int userNum) {
		System.out.println("MypagerDAO 에서 유저가 작성한 총 댓글 수 조회  전" );
		int result = sqlSession.selectOne("mypage.getTotalCommentNum", userNum);
		System.out.println("MypagerDAO 에서 유저가 작성한 총 댓글 수 조회 결과 : " + result);
		return result;
	}
	
	// 해당 유저가 작성한 댓글 모두 가져오기 !!!만들어야함 !! 만들고 지울것!!!
	public List<MyTotalCommentDTO> selectAllMyComment(Map<String, Integer> pageMap) {
		System.out.println("========selectAll : 유저가 작성한 댓글 가져오기");
		List<MyTotalCommentDTO> myList = sqlSession.selectList("mypage.selectAllMyComment", pageMap);
		System.out.println("가져온 목록 : " + myList);
		return myList;
	}
