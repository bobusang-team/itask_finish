...
public class AskQualController implements Execute {
	...
	public Result execute(HttpServletRequest request, HttpServletResponse response)
		...
		// 전체 회원 목록 가져오기
		List<ArticleAdminDTO> askQual = adminDAO.AskQual(pageMap);
		
		// 회원 목록을 request 객체에 담아 JSP로 전달
		request.setAttribute("askQual", askQual);
		
		int total = adminDAO.getAskQualTotal();
...
//	      startPage = Math.max(startPage, 0);
		...
		System.out.println("boardList : " + askQual);
		System.out.println(
				"startPage : " + startPage + ", endPage : " + endPage + ", prev : " + prev + ", next : " + next);
		System.out.println("===================");
		// 결과 페이지 설정
		Result result = new Result();
		result.setPath("/app/admin/askQual.jsp");
		return result;
	}
