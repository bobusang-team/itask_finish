...
public class GeneralMemberController implements Execute {
	...
	public Result execute(HttpServletRequest request, HttpServletResponse response)
		...
		// 전체 회원 목록 가져오기
		System.out.println("회원목록 가져오기전");
		List<UserAdminDTO> users = adminDAO.GeneralMember(pageMap);
		System.out.println("목록가져옴" + users);
		...
		int total = adminDAO.getGeneralTotal();
...
//	      startPage = Math.max(startPage, 0);
		...
		System.out.println("===================");
		// 결과 페이지 설정
		result.setPath("/app/admin/generalMember.jsp");
