...
public class TipSecController implements Execute {
	...
	public Result execute(HttpServletRequest request, HttpServletResponse response)
		...
		// 전체 회원 목록 가져오기
		List<ArticleAdminDTO> tipSec = adminDAO.TipSec(pageMap);

		// 회원 목록을 request 객체에 담아 JSP로 전달
		request.setAttribute("tipSec", tipSec);
		
		int total = adminDAO.getTipSecTotal();
...
//	      startPage = Math.max(startPage, 0);
		...
		System.out.println("boardList : " + tipSec);
		System.out.println(
				"startPage : " + startPage + ", endPage : " + endPage + ", prev : " + prev + ", next : " + next);
		System.out.println("===================");
		// 결과 페이지 설정
		Result result = new Result();
		result.setPath("/app/admin/tipSec.jsp");
		return result;
	}
