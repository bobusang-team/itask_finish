...
public class BlackListController implements Execute{
	...
	public Result execute(HttpServletRequest request, HttpServletResponse response)
		...
		List<BlackListDTO> users = adminDAO.BlackList(pageMap);
		for (BlackListDTO user : users) {
			System.out.println("블랙리스트 회원 데이터: " + user);
		}
		// 회원 목록을 request 객체에 담아 JSP로 전달
		request.setAttribute("users", users);
		
		int total = adminDAO.getBlackTotal();
...
//	    startPage = Math.max(startPage, 0);
		...
		// 결과 페이지 설정
		result.setPath("/app/admin/blackList.jsp");
