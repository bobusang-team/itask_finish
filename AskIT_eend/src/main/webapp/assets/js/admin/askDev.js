...
function submitEditForm() {
	...
	const selectedUsers = document.querySelectorAll('.item-checkbox:checked');
		...
		if (confirmEdit) {
			const articleNum = selectedUsers[0].closest('.askdevList').querySelector('.articleNum').value;
...
function submitDeleteForm() {
	...
	console.log(document.getElementById('articleTopCate').value);
	...
	userIdsInput.value = Array.from(selectedUsers).map(user => 
	    user.closest('.askdevList').querySelector('.articleNum').value
	).join(",");
