...
<body>
	...
	<main class="itaskBox">
		...
		<article class="askDetail-elements">
			...
			<main class="askBox">
				...
				<div class="ask-topcontainer">
					...
					<!-- 자격증 태그 버튼 -->
					<div class="ask-tag">
						<button class="ask-tag-btn" onclick="setTag('NEWWORK')">NEWWORK</button>
						<button class="ask-tag-btn" onclick="setTag('WEB')">WEB</button>
						<button class="ask-tag-btn" onclick="setTag('MOBILE')">MOBILE</button>
						<button class="ask-tag-btn" onclick="setTag('악성코드')">악성코드</button>
						<button class="ask-tag-btn" onclick="setTag('포렌식')">포렌식</button>
						`
					</div>
				</div>
			</main>
			...
			<article class="ask-text-total" data-tag="${articleBotCate}">
				<div class="ask-text-header">
					<span class="ask-text-tag">${article.articleTagName}</span> <input
						type="text" class="ask-text-title"
				...
				<article class="text-textbottom">
					...
					<div class="Detail-content">
						...
						<span id="Detail-viewcount">${article.articleView }</span>
						...
						<div class="askDetail-comment-show">
							...
							<div class="askDetail-comment-info">
								...
								<span class="askDetail-comment-monitor"> <img
									src="./../asset0.s/img/moniter.png"
									alt="사진오류"> ${comments.commentMonitorNum}</span> 
									<span id="commentDelete" class="askDetail-comment-delete">삭제</span>
							</div>
						</div>
						<!-- --------------------------이만큼 추가돼야함^_^~-------------------- -->
					</div>
		...
		<form id="articleForm">
			<input type="hidden" id="articleBotCate"
				value="${article.articleBotCate}"> <input type="hidden"
				id="articleTagName" value="${article.articleTagName}">
		</form>
		<!-- 삭제용 Form -->
		<form id="deleteForm"
			...
			<input type="hidden" name="articleNums" id="articleNums"
				...
				name="articleTopcate" id="articleTopcate"
				value="${article.articleTopCate }"> <input type="hidden"
				name="articleBotcate" id="articleBotcate"
				value="${article.articleBotCate }">
		</form>
		<!-- 수정용 Form -->
		<form id="articleEdit"
			action="${pageContext.request.contextPath}/admin/updateArticle.ad"
			method="post">
			<input type="hidden" name="articleNum" id="articleNum"
				value="${article.articleNum }"> <input type="hidden"
				name="articleTopcate" id="updateTopcate"
				value="${article.articleTopCate } "> <input type="hidden"
				name="articleBotcate" id="updateBotcate"> <input
				type="hidden" name="articleTagName" id="updateTagName"> <input
				type="hidden" name="articleTitle" id="updateTitle"> <input
				type="hidden" name="articleText" id="updateText">
		</form>
		<!-- 댓글삭제용 Form -->
