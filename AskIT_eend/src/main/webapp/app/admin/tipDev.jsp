...
<body>
	...
	<main>
			...
			<c:forEach var="tipDev" items="${tipDev}">
				<div class="tipdevList">
					<div>
						...
						<input type="hidden" class="articleNum" value="${tipDev.articleNum }">
						<input type="hidden" name="articleTopCate" id="articleTopCate" value="${tipDev.articleTopCate }">
						<input type="hidden" name="articleBotCate" id="articleBotCate" value="${tipDev.articleBotCate }">
						<p>${tipDev.articleTagName}</p>
						<a href="${pageContext.request.contextPath}/admin/articleDetail.ad?articleNum=${tipDev.articleNum}">
      ...
      <c:if test="${prev}"> <!-- 아주 처음으로 가는 버튼 -->
         <a href="${pageContext.request.contextPath}/admin/tipDev.ad?page=1"
                  class="firstPage">«</a>
     </c:if>
     ...
     <c:set var="realStartPage" value="${startPage < 0 ? 0:startPage }" />
            ...
            <c:choose>
               <c:when test="${!(i == page)}">
                  <a href="${pageContext.request.contextPath}/admin/tipDev.ad?page=${i}">
                           <c:out value="${i}" />
                  </a>
               </c:when>
               <c:otherwise>
      ...
      <c:if test="${next}"> <!-- 다음 페이지가 있다면 다음으로 넘어가는 버튼 생성 -->
            <a href="${pageContext.request.contextPath}/admin/tipDev.ad?page=${endPage + 1}"
                  class="next">›</a>
      </c:if>
      <c:if test="${next}"> <!-- 완전 끝으로 가는 버튼 생성 -->
            <a href="${pageContext.request.contextPath}/admin/tipDev.ad?page=${realEndPage}"
                  class="realEndPage">»</a>
      </c:if>
